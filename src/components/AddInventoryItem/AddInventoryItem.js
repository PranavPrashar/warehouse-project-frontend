...
function AddInventoryItem() {
  ...
  const [errorClass, setErrorClass] = useState(
    "add-inventory-form__error--hidden"
  );
  const [category, setCategory] = useState(null);
  const [unfilteredCategory, setUnfilteredCategory] = useState(null);
    ...
    axios
      ...
      .then(
        axios.spread((...responses) => {
          ...
          setWarehouses(responses[0].data);
          setInventory(itemCategories);
          setCategory(filteredCategories);
          setUnfilteredCategory(itemCategories);
        })
      );
  }, []);
  return (
    <main className="add-inventory">
      ...
      >
        ...
        <section className="add-inventory-form__button-wrapper">
          <button
            onClick={handleCancelClick}
            className="add-inventory-form__cancel-button"
          >
            Cancel
          </button>
          <button
            form="add-inventory-form"
            className="add-inventory-form__add-button"
          >
            + Add Item
          </button>
        </section>
      </form>
      <h2 className={`${errorClass}`}>
...
//       !event.target[6].value
//     ) {
//       setErrorClass("add-inventory-form__error");
//       // this.setState({
//       //   errorClass: "add-inventory-form__error",
//       // });
//     }
