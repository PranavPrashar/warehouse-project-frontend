...
class InventoryListComponent extends Component {
  ...
  setSelectedInventory = (inventoryId) => {
    const selectedInventory = this.state.inventories.find((inventory) => inventoryId === inventory.id)
    this.setState({
        ...
        <section className="inventory__page__header">
          ...
          <div className="inventory__page__header--search">
            <img
              ...
              alt="Search Icon"
            />
            <input placeholder="Search..." className="inventory__page__header--search--box" />
          </div>
          <button onClick={handleAddNewItemClick} className="inventory__page__header--new--item">
            {" "}
            + Add New Item
          </button>
        </section>
        <section className="inventory__page__titles">
          <h4 className="inventory__page__titles--header">inventory Item
            <img src={sortIcon} alt="Sort Icon" />
          </h4>
          <h4 className="inventory__page__titles--header">category
            <img src={sortIcon} alt="Sort Icon" />
          </h4>
          <h4 className="inventory__page__titles--header">status
            <img src={sortIcon} alt="Sort Icon" />
          </h4>
          <h4 className="inventory__page__titles--header">qty
            <img src={sortIcon} alt="Sort Icon" />
          </h4>
          <h4 className="inventory__page__titles--header">warehouse
            <img src={sortIcon} alt="Sort Icon" />
          </h4>
          <h4 className="inventory__page__titles--header">actions</h4>
        </section>
        <section className="inventory__page__content">
          <InventoryListCardComponent selectedInventory={this.state.selectedInventory} setSelectedInventory={this.setSelectedInventory} inventories={this.state.inventories} />
        </section>
      </main>
    );
  }
